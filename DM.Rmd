...
DB %>%
  ...
  rename(Number_Periods_available = n,
         Number_of_loans = nn) %>% 
  mutate(percent = 100 * (Number_of_loans / 346724),
         cumulative_sum = cumsum(Number_of_loans),
         cumulative_percent = 100 * (cumulative_sum / 346724)) %>%
  mutate_all(., ~round(., 3)) %>% 
  collect()
```
...
Loans_by_MSA <- 
  ...
  mutate(Percent = round(100 * (LoansPerMSA / 346724), 2))
...
```{r chunk13, fig.height=4, fig.width=8}
...
```{r chunk14, echo=FALSE, fig.height=4, fig.width=8}
...
Moments_stats(within_transformation_graph, confirmed, 'Confirmed Within') %>% 
  ...
  left_join( Moments_stats(between_transformation_graph, Delinquency_Status, 'Delinquency_Status Between'), by =' ') %>%   mutate_if(is.numeric, ~round(., 3)) %>% 
  kable( booktabs = T) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
```{r chunk18 }
graph_chunk18 <- boxplot_plot(within_transformation_graph, confirmed, 'Confirmed within ') +
...
ggsave(file="chunk18.png", dpi=400, width=8, height=4, units='in')
```

\begin{figure}[H]
\begin{center}
\includegraphics{chunk18.png}
\caption{Boxplot for Confirmed Covid cases and Delinquency Status considering within and between transformation}
\end{center}
\end{figure}
...
within_transformation_graph %>% 
  ...
  geom_boxplot() +
  coord_flip() +
  labs(title = 'Boxplot for the Delinquency Status variable for 20 random individuals',
       y = 'Loan_Seq_Number')
```
\
...
within_transformation_graph %>% 
  ...
  geom_boxplot() +
  coord_flip() +
  labs(title = 'Boxplot for the E_LoanToValue variable for 20 random individuals', 
       y = 'Loan_Seq_Number')
```
\
...
within_transformation_graph %>% 
  ...
  geom_boxplot() +
  coord_flip() +
  labs(title = 'Boxplot for the Confirmed variable for 20 random individuals',
       y = 'Loan_Seq_Number')
```
\
...
within_transformation_graph %>% 
  ...
  select(-Loan_Seq_Number, -Reporting_Period, - MSA, -O_DebtToIncome,
         -O_LoanToValue,-Lender_Type,-Fintech, -Credit_score) %>% 
datasummary_correlation(title = 'Correlation matrix considering a within transformation', fmt = 3) %>%
kable_styling(latex_options = c("striped", "hold_position")) %>%
row_spec(0, angle = 90)
```
...
datasummary_correlation(title = 'Correlation matrix considering a between transformation', fmt = 3) %>%
...
within_cor_trend <- within_transformation_graph %>% 
  mutate(Reporting_Period = as.numeric(as.factor(Reporting_Period)),
         MSA = as.factor(MSA),
         Seller_Name = as.factor(Seller_Name)) %>% 
  select(-Loan_Seq_Number, -MSA, -Seller_Name, - Credit_score, -Lender_Type, -Fintech,
...
```{r chunk31}
# Very slow ...
within_transformation_graph[c(1:50000),] %>% 
  filter(Current_UPB > -15000) %>% 
  drop_na(confirmed, Current_UPB) %>% 
  plot_biv(x = confirmed, y = Current_UPB) +
  labs(title = "Bivariate graph for the within transformation of Confirmed and Current UPB variable",
       subtitle = 'Blue line : linear, Red line : loess, Orange line : cubic')
```
...
within_transformation_graph[c(1:50000),] %>% 
  ...
  labs(title = "Bivariate graph for the within transformation of Confirmed and Delinquency_Status variable",
       subtitle = 'Blue line : linear, Red line : loess, Orange line : cubic')
```
...
kable(tidy(phtest(plmrandom, plmwithin)), caption=
 "Hausman endogeneity test for the random effects wage model",  booktabs = T)
```
...
```{r chunk41, eval=FALSE, include=FALSE}
pht <- plm(Delinquency_Status ~ confirmed + E_LoanToValue + Credit_score | E_LoanToValue + confirmed,
...
between_transformation %>% 
  ...
  ggtitle('Between transformation')
```
...
modelsummary(list(res_fe, res_hfe), gof_omit = "Pseudo|IC|Log", stars = TRUE,
             title ='Fixed effects model with ODA / GNI as dependent variable') %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
modelsummary(list(res_fe_2, res_hfe_2), gof_omit = "Pseudo|IC|Log", stars = TRUE,
             title = 'Fixed effects model with Corruption index as dependent variable') %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
modelsummary(list(res_fe_3, res_hfe_3), gof_omit = "Pseudo|IC|Log", stars = TRUE,
             title = 'Fixed effects model with the growth rate of GDP per capita as dependent variable') %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
```{r chunk72, fig.height=4, fig.width=8}
...
\
We begin by ploting the between transformation over the full period. The between growth rate of ODA can have negative values because we're considering Net ODA. Moreover, our between transformed variables are likely skewed.
\
...
\
And then, plotting the within transformation for *oda_gni, g_oda_net, gdp_per_cap* variables.
Again, with the within transformation, it more likely that our variables are normally distributed around 0 even though, concerns about high kurtosis are there. 
\

```{r out.width = '55%'}
panel_data %>% 
  select(country, year, oda_gni, g_oda_net, gdp_per_cap) %>% 
  group_by(country) %>% 
  mutate(across(where(is.double),~ . -  mean(., na.rm = T))) %>% 
